import { DetectedEntity, SensitiveCategory } from '../types.js';
...
interface DetectionRule {
  ...
  regex: RegExp;
  confidence: number;
  extractor?: (match: RegExpExecArray, text: string) => { value: string; start: number; end: number } | null;
}
  ...
  private rules: DetectionRule[] = [
    ...
    // 6. Personal Identifiers (Names / Identity)
    {
      ...
      extractor: (match) => {
        ...
        if (ignore.includes(val.toLowerCase().trim())) return null;
    ...
    },
    {
      ...
      extractor: (match) => {
        ...
        if (ignoreList.includes(loc.toLowerCase())) return null;
        return { value: match[0], start: match.index, end: match.index + match[0].length };
      },
    },
  ...
  public detect(text: string): DetectedEntity[] {
    ...
    for (const rule of this.rules) {
      ...
      while ((match = rule.regex.exec(text)) !== null) {
        ...
        const isOverlapping = detections.some(
          (d) =>
            (extracted.start >= d.startIndex && extracted.start < d.endIndex) ||
