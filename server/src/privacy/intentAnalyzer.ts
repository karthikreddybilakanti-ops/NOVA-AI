export interface TaskIntent {
  domain: 'banking' | 'programming' | 'writing' | 'science' | 'math' | 'casual' | 'general';
  summary: string;
  problemType: string;
...
export class IntentAnalyzer {
  public static analyze(rawPrompt: string): TaskIntent {
    ...
    if (
      (p.includes('intro') || p.includes('introduction') || p.includes('bio') || p.includes('about me')) &&
    ...
    if (
      /^(?:hi|hello|hey|good\s+\w+)/i.test(p) &&
      p.split(/\s+/).length <= 6
    ) {
      return {
        domain: 'casual',
